...
for(var line_count=0;line_count<len_lines;line_count++)                  //traversing each and every line
{
  ...
  if(lines[line_count].search("Oilseeds") != -1)                        //if found the oilseed details
  {
    var words = lines[line_count].split(/,(?=(?:(?:[^"]*"){2})*[^"]*$)/);
    tmp["crop_type"]=(words[0].substring(33,words[0].length));
...
fs.writeFile('test1.json', JSON.stringify(json) , function (err)
{
